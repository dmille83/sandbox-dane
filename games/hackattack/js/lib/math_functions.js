...
function getCrossProduct(x1, y1, x2, y2)
{
    return (x1*y2) - (y1*x2);
}
...
/*
// SOURCE:  http://stackoverflow.com/questions/8017541/javascript-canvas-collision-detection
...
*/
// SOURCE:  https://github.com/jakesgordon/javascript-tiny-platformer/
function rectangleIntersect(x1, y1, w1, h1, x2, y2, w2, h2) {
	return !(((x1 + w1 - 1) < x2) ||
		     ((x2 + w2 - 1) < x1) ||
		     ((y1 + h1 - 1) < y2) ||
...
function getAngleOffset(angle1, x1, y1, x2, y2)
{
	...
	var offsetAngle = angleFromOrigin - angle1 - 180 % 360;
...
function getActorDistance(actor1, actor2) {
	return getDistance(actor1.x, actor1.y, actor2.x, actor2.y);
}

function getActorAngleOffset(actor1, actor2) {
	//console.log(actor1.angle +" "+ actor1.x +" "+ actor1.y +" "+ actor2.x +" "+ actor2.y);
	//console.log(getAngleOffset(actor1.angle, actor1.x, actor1.y, actor2.x, actor2.y));
	return getAngleOffset(actor1.angle, actor1.x, actor1.y, actor2.x, actor2.y);
}


// function getActorCrossProduct(monster, victim)
// {
// 	return (monster.xspeed*victim.y) - (monster.yspeed*victim.x);
// }
