...
// SOURCE:  http://stackoverflow.com/questions/17456783/javascript-figure-out-point-y-by-angle-and-distance
function findPointFromAngleDistance(x, y, angle, distance) {
    var result = {};
    result.x = Math.round(Math.cos(angle * TO_RADIANS) * distance + x);
    result.y = Math.round(Math.sin(angle * TO_RADIANS) * distance + y);
    return result;
}
