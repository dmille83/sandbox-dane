...
sagscroller.prototype={
	...
	addloadingpanel:function($, mode){
		...
		if (mode=="preload"){
			...
			this.$loadingimg=$(loadingimg).css({position:'absolute', zIndex:13})
		}
		else{
			var sliderdimensions=[this.$slider.width(), this.$slider.height()]
			var $loadingpanel=$('<div />').css({position:'absolute', left:0, top:0, background:'black', opacity:0.5, width:sliderdimensions[0], height:sliderdimensions[1], zIndex:12}).appendTo(this.$slider)
	...
	addnavpanel:function(){
		...
		var $navpanel=$('<div class="sliderdesc"><div class="sliderdescbg"></div><div class="sliderdescfg"><div class="sliderdesctext"></div></div></div>')
			.css({position:'absolute', width:'100%', left:0, top:-1000, zIndex:'11', visibility:'hidden'})
			...
			.eq(1).css({color:'white', top:'-5px'}).end() //"sliderdescfg" div
		...
		$navpanel.css({top:'calc(100% - '+parseInt(sagscroller_constants.navpanel.height)+')', height:sagscroller_constants.navpanel.height}).find('div').css({height:'100%'})
	},

	resetuls:function(){ //function to swap between primary and secondary ul
		var $tempul=this.$mainul
		this.$mainul=this.$secul.css({zIndex:10})
		this.$secul=$tempul.css({zIndex:9})
	...
	init:function($){
		...
		this.$mainul=this.$slider.find('ul:eq(0)').css({zIndex:10})
		this.$lis=this.$mainul.find('li')
		if (setting.navpanel.show)
			this.addnavpanel()
		this.$secul=this.$mainul.clone().css({top:this.$mainul.height(), zIndex:9}).appendTo(this.$slider) //create sec UL and add it to the end of main UL // !!!
